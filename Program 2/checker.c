...
struct nodes{
  char toVisit[2000][4096];  // directory addresses left to search
  int nodesWithMulti[10000]; // inode #s who are multilinked and we've seen b4
...
void addDirectory(struct nodes *nodes, char* addr){  
  ...
  if(nodes->visitsLeft > 2000){
    fprintf(stderr, "ERROR: The number of queued directories is greater than the program can handle (2000):(\n");
  }
...
void checkLink(char* addr, struct fileData *data){
  ...
  if (returnSize = readlink(addr, buffer, 4096) != -1){
    if(stat(buffer, &temp_sb) != 0){
      data->invalidLink++;
    }
  }
}
...
int handleInode(struct stat ino, struct fileData *data, struct nodes *nodes, char* addr){
  ...
  if( ((ino.st_mode & S_IFMT) != S_IFDIR) && ino.st_nlink>1){
    ...
    // Check if this multiLink file has been viewed before
    int dontCount=0;
    for (int i=0; i<data->multiLink; i++){
  ...
  switch (ino.st_mode & S_IFMT) {
        ...
        case S_IFREG:
          data->regCount++;
          data->fileSize = data->fileSize+(ino.st_size);
          data->allocatedBlocks = data->allocatedBlocks+(ino.st_blocks);
          break;
...
void printData(struct fileData data){
  ...
  ", data.fileSize, data.allocatedBlocks);
